...
public class HologramManager {
    ...
    public HologramManager() {
        hologramHashMap.put(ArenaType.EXPERIMENTAL,createHologram(new ItemBuilder(Material.SKULL_ITEM,1,(byte)3).build(), BukkitHolyAPI.getInstance().getLocationManager().getLocation("multireduce")));

        // multi reduce hats
        Hologram hologram2 = HologramsAPI.createHologram(Clutches.getInstance(),BukkitHolyAPI.getInstance().getLocationManager().getLocation("multireduce").clone().add(0 + 0.5,3.35,0));
        hologram2.appendItemLine(new ItemBuilder(Material.SKULL_ITEM,1,(byte)3).build());
        hologram2.appendTextLine(null);

        Hologram hologram3 = HologramsAPI.createHologram(Clutches.getInstance(),BukkitHolyAPI.getInstance().getLocationManager().getLocation("multireduce").clone().add(0 - 0.5,3.35,0));
        hologram3.appendItemLine(new ItemBuilder(Material.SKULL_ITEM,1,(byte)3).build());
        hologram3.appendTextLine(null);


/*        Hologram plholo = HologramsAPI.createHologram(Clutches.getInstance(),BukkitHolyAPI.getInstance().getLocationManager().getLocation("playground").clone().add(0,3.35,0));
        ...
        hologramHashMap.put(ArenaType.REDUCE,createHologram(new ItemBuilder(Material.STICK,1).build(), BukkitHolyAPI.getInstance().getLocationManager().getLocation("reduce")));
        hologramHashMap.put(ArenaType.CLUTCH,createHologram(new ItemBuilder(Material.SANDSTONE,1,(byte)1).build(), BukkitHolyAPI.getInstance().getLocationManager().getLocation("clutch")));
        hologramHashMap.put(ArenaType.DIAGONAL_CLUTCH,createHologram(new ItemBuilder(Material.SANDSTONE_STAIRS,1,(byte)1).build(), BukkitHolyAPI.getInstance().getLocationManager().getLocation("diagonalclutch")));
    }
